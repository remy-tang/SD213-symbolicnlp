...
vp(FV,PVP,TVP) --> v(FV,PV,TV), dp(FDP,PDP,TDP), dp(FDP2,PDP2,TDP2), { % ditransitive verb, eg. 'give'
		...
		link(2, PV,PDP,PVP),	% performs some variable unification
		link(3, PV,PDP2,PVP),	% performs some (other) variable unification
		TVP  = vp(TV, TDP, TDP2) }.

vp(FV,PVP,TVP) --> v(FV,PV,TV), pp(FDP,PDP,TDP), pp(FDP2,PDP2,TDP2), { % ditransitive verb with two indirect complements, eg. 'talk'
...
n([gloss:left, num:sing],left(_,_),n(left)) --> [left].
